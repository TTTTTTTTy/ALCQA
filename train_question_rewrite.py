...
question_path = 'data/pretrain/decomposed.txt'
model_save_path = 'saved_models/question_decompose/'
if not os.path.exists(model_save_path):
    os.makedirs(model_save_path, exist_ok=True)
log_path = 'logs/question_decompose/'
...
dataset = []
for line, sub_question in zip(open(data_path).readlines(), open(question_path).readlines()):
    d = json.loads(line)
    ...
    for mask, m_name in d['mask_name'].items():
        ...
        input_text.append('</%s>' % re.sub(r'[0-9]', '', mask.lower()))
    dataset.append((' '.join(input_text), sub_question.replace('[SEP]', '#').strip()))
...
for epoch_idx in range(epoch_num):
    ...
    model_save_file = os.path.join(model_save_path, 'epoch_%d_score_%.4f.bin' % (epoch_idx+1, mean_bleu_score))
    torch.save(model.state_dict(), model_save_file)
    logger.info('save new model checkpoints to [%s]' % (model_save_file))